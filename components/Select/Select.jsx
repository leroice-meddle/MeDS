...
function Select({ label = 'Label', description = '', placeholder = 'Select...', options = [], value: controlledValue, hasLabel = true, hasDescription = false, hasError = false, error = 'Error', state = 'default', onChange }) {
  const [open, setOpen] = useState(false)
  const [selected, setSelected] = useState(controlledValue || null)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontFamily: 'Geist, sans-serif', width: '100%', position: 'relative' }} ref={ref}>
      {hasLabel && <p style={{ fontSize: '16px', fontWeight: 400, lineHeight: 1.4, letterSpacing: '-0.16px', color: isDisabled ? '#b3b3b3' : '#1e1e1e', margin: 0 }}>{label}</p>}
      {hasDescription && description && <p style={{ fontSize: '16px', fontWeight: 400, lineHeight: 1.4, letterSpacing: '-0.16px', color: isDisabled ? '#b3b3b3' : '#757575', margin: 0 }}>{description}</p>}
      <div
      ...
      {open && !isDisabled && (
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, background: 'white', border: '1px solid #e6e6e6', borderRadius: '8px', padding: '8px', boxShadow: '0 1px 2px rgba(12,12,13,0.05), 0 1px 2px rgba(12,12,13,0.1)', zIndex: 10, display: 'flex', flexDirection: 'column', gap: '4px' }}>
          {options.map((opt, i) => {
            ...
            const isSelected = selected && (typeof selected === 'object' ? selected.value === optValue : selected === optValue)
            return (
              <div key={i} onClick={() => handleSelect(opt)}
                style={{ padding: '8px 8px', borderRadius: '6px', fontSize: '16px', fontWeight: isSelected ? 500 : 400, color: '#1e1e1e', cursor: 'pointer', background: isSelected ? '#f5f5f5' : 'transparent', letterSpacing: '-0.16px', lineHeight: 1.4, fontFamily: 'Geist, sans-serif' }}>
                {optLabel}
              </div>
            )
          })}
      ...
      {isError && hasError && <p style={{ fontSize: '16px', fontWeight: 400, lineHeight: 1.4, letterSpacing: '-0.16px', color: '#900b09', margin: 0 }}>{error}</p>}
    </div>
  )
}
